...
import { FiDollarSign, FiExternalLink, FiHelpCircle, FiLogOut, FiUser } from 'react-icons/fi';
import { useLocation } from 'react-router-dom';
import { Box, Icon, Link as ChakraLink, Portal } from '@chakra-ui/react';

import { Container, NavBar, Option } from '../../components';
...
function Profile() {
  ...
  const containerComponent = (overlay: any) => {
    switch (overlay) {
      ...
      case 4:
        return <LegalStructure goBack={handleClose} />;
      case 5:
  ...
  return (
    <Fragment>
      <NavBar />
      <Container ref={ref}>{containerComponent(overLay)}</Container>
    </Fragment>
  );
}
...
export const Account = ({
  ...
  return (
    <Box mt={8}>
      <Overline ml={3} mb={2} display={popOver ? 'none' : 'block'}>
        Account
      </Overline>
      <Box as="ul" borderRadius="2xl" overflow="hidden">
...
export const Legal = ({ popOver }: { popOver: boolean }) => (
  <Box mt={popOver ? 0 : 8}>
    ...
    >
      <Option icon={false}>
        <Icon as={FiExternalLink} w={4} h={4} mr={4} verticalAlign="baseline" />
