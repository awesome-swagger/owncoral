...
export const ResidentialAddress:React.FC<StepPropsT> = ({ nextStep, prevStep }) => {
  const {
    ready,
    value,
    setValue,
  ...
  useEffect(() => {
    ...
    setValue(formState?.step2 || '', false);

    handleFormattedAddress(formState?.step2);
  ...
  const handleSubmit = () => {
    if (formattedAddress) {
      form.dispatch?.({ step2: Object.values(formattedAddress).join(', ') });
      nextStep()
    }
  }
