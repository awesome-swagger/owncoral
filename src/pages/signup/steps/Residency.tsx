...
export const Residency:React.FC<StepPropsT> = ({ nextStep, prevStep }) => {
  const { dispatch, signupInfo } = useContext(SignupContext);
  const [country, setCountry] = useState<string>(signupInfo?.residencyCountryCode || "");

  const handleSubmit = useCallback(
    () => {
      dispatch?.({ residencyCountryCode: country });
      nextStep(country === 'US' ? 0 : 1);
    },
    [country, dispatch, nextStep],
  );
